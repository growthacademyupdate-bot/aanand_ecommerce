...
async function main() {
  const paymentsRes = await fetch('https://api.razorpay.com/v1/payments?count=100&status=captured', {
  ...
  console.log('razorpay_captured_count', captured.length);
  ...
  try {
    ...
    const missingPaid = matched.filter((m) => m.orderStatus !== 'confirmed' || m.paymentStatus !== 'paid');
    console.log('matched_nonconfirmed_or_nonpaid', missingPaid.length);
    if (missingPaid.length > 0) {
      console.log('matched_nonconfirmed_or_nonpaid_samples', JSON.stringify(missingPaid.slice(0, 20), null, 2));
    }
  } finally {
    await client.close();
  }
